...
class ATTKN(object):
    ...
    def cal_logit(self, query_seq,query_num,query_mask, query_emb, query_lastword, boxes_embedding, label_embedding,\
        ...
        logit = self.add_fc_layers(concat_out, name='dense', mode=mode)
    ...
    def model_fn(self, features, labels, mode, params):
        ...
        logit,sfotmax_score = self.cal_logit(query_seq,query_num,query_mask, query_emb, query_lastword, boxes_embeddings[0], label_embeddings[0],boxes_num[0],
        ...
        if neg_num > 0:
            ...
            for i in range(1, neg_num + 1):
                logit,sfotmax_score = self.cal_logit(query_seq,query_num,query_mask, query_emb, query_lastword, boxes_embeddings[i], label_embeddings[i],boxes_num[i],
            ...
            beta = 0.5**(1+iteration/stepsize)
            
            # beta = tf.reduce_max([beta,0.1])            
            ...
            # beta = 0.1
            loss = (1-beta)*hard_loss + beta * soft_loss
